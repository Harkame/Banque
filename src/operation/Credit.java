package banque_executable.operation;

import banque_executable.compte.*;
import banque_executable.compte.remunere.CompteRemunere;
import banque_executable.compte.remunere.PlanEpargneBloque;
import banque_executable.exception.OperationBancaireException;
/**
 * 
...
public class Credit extends Operation {
	...
	/**
	 * Fait passer le statut du Credit (this) a OK, si le credit est possible, sinon
	 * leve une exception
	 * 
	...
	public void CreditPossible() throws OperationBancaireException {
		if (this.statut == Statut.Attente) {
			...
			if (this.getCompte() instanceof CompteRemunere) { // Si on a affaire
																// a
																// un compte
				...
				if (Depasse(this.getCompte(), this.getMontant())) {
					...
					throw new OperationBancaireException(
							"Solde plafond "
									+ ((CompteRemunere) this.getCompte())
											.getSoldePlafond() + " depasse");
				}

			}
			if (this.getCompte() instanceof PlanEpargneBloque) {
				if (this.getDate() != ((PlanEpargneBloque) this.getCompte())
						.getDate()) { // Je ne savais pas comment dire que la
										// date de l'operation devait être <
										// celle autorise
					this.statut = Statut.KO;
