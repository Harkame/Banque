package banque_executable.main;

import banque_executable.compte.*;
import banque_executable.compte.remunere.CompteEpargne;
import banque_executable.exception.*;
import banque_executable.operation.*;
import banque_executable.ordre_virement.*;
import banque_executable.personne.*;
/**
 * 
...
public abstract class Banque {
	...
	public static void main(String[] Args) throws OperationBancaireException,
			PersonnelNonAutoriseException {
		System.err
				.println("Il est possible que certain messages, nottement d'erreure ne se place pas au bon endroit, si cela arrive, veuillez relancer le programme, merci");
		AffNull();
		System.out.println("------------------------------------");
		...
		System.out.println("------------------------------------------------");
		System.out
				.println("Selon la vitesse de relance du programme, les message d'alerte ne se situe pas au même endroit");
		System.out
				.println("Ces messages se trouve dans des bloc catch, ces dernier sont secondaire par rapport au reste du programme");
		System.out
				.println("La memoire va d'abord s'occuper du code, puis des bloc catch");
		AffNull();
		System.out
				.println("---------------------------------------------------------------------");
		System.out
				.println("--- Subsidiaire - Validation manuelle d'une operation en attente  ---");
		System.out
				.println("---------------------------------------------------------------------");
		AffNull();
